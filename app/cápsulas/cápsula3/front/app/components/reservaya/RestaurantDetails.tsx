...
import { getRestaurantData } from "../../data/restaurantsData";
...
export function RestaurantDetails({
  ...
  // Obtener datos del restaurante seleccionado
  const restaurantData = getRestaurantData(selectedRestaurantId || null);

  // Si no hay datos, usar valores por defecto
  const restaurantName = restaurantData?.name || "King Halo";
  const restaurantType = restaurantData?.type || "Restaurante";
  const restaurantRating = restaurantData?.rating || 4.8;
  const restaurantHours = restaurantData?.hours || "11:00 AM - 10:00 PM";
  const restaurantAddress =
    restaurantData?.address || "Av. Principal 123, Centro";
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-orange-50 via-amber-50 to-red-50">
      ...
      <div className="bg-gradient-to-r from-orange-500 to-red-500 px-6 py-4 shadow-lg sticky top-0 z-50">
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            <h2 className="text-white">{restaurantName}</h2>
            <div className="flex items-center gap-3 mt-1">
              <div className="flex items-center gap-1">
                <Star className="h-4 w-4 fill-yellow-300 text-yellow-300" />
                <span className="text-white/90">{restaurantRating}</span>
              </div>
              <span className="text-white/70">•</span>
              <span className="text-white/90">{restaurantType}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="p-6 pb-32">
        ...
        <Card className="mb-6 border-2 border-orange-200 rounded-2xl overflow-hidden">
          <div className="p-5 bg-white">
            <div className="flex items-start gap-4 mb-4">
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-slate-700">{restaurantHours}</span>
                </div>
                <div className="flex items-center gap-2">
                  <MapPin className="h-5 w-5 text-orange-500" />
                  <span className="text-slate-700">{restaurantAddress}</span>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="mt-4">
                <label className="text-slate-600 mb-2 block">Fecha</label>
                <input
                  type="date"
                  value={selectedDate}
        ...
        {/* Reviews Section */}
        <ReviewsSection restaurantName={restaurantName} />
      </div>
      ...
      {totalItems > 0 && (
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t-4 border-orange-300 shadow-2xl p-6 animate-in slide-in-from-bottom-4">
              ...
              onClick={() => {
                if (!isLoggedIn) {
                  toast.error("Debes iniciar sesión", {
                    description:
                      "Inicia sesión o crea una cuenta para hacer reservas",
                    duration: 2000,
                  });
                  setTimeout(() => {
                    onLogin();
          ...
          <p className="text-slate-500 text-center">
            Reserva para {guests} {guests === 1 ? "persona" : "personas"} a las{" "}
            {selectedTime} el {formatDate(selectedDate)}
          </p>
        </div>
      )}
    </div>
